import { p as pipelineSymbol, A as AstroError, d as ActionCalledFromServerError } from './sequence_DK-jjJFr.mjs';
import { c as createActionsProxy, d as defineAction } from './entrypoint_BZjGfNhw.mjs';
import * as z from 'zod/v4';
import { d as db, D as DonationInfo, G as Guest, a as GalleryImage, S as Settings, T as Table } from './_astro_db_8BCavLB5.mjs';
...
function nameMatches(inputName, guestName) {
...
const server = {
  rsvp: {
    submitByName: defineAction({
      ...
      handler: async (input) => {
        ...
        if (match.status !== "pending") {
          const alreadyMsg = match.status === "confirmed" ? "¡Ya confirmaste tu asistencia anteriormente! 🎉 Nos vemos en la boda." : "Ya indicaste que no podrías asistir. Si necesitas cambiar tu respuesta, contáctanos directamente.";
          const groupNamesMsg2 = groupGuests.length > 1 ? `<br/><small>Invitados marcados: ${groupGuests.map((g) => g.name).join(", ")}</small>` : "";
          return {
            success: true,
            alreadyResponded: true,
            guestName: match.name,
            message: alreadyMsg + groupNamesMsg2,
            bibleVerse: null
          };
        }
    ...
    submit: defineAction({
      ...
      handler: async (input) => {
        ...
        await db.update(Guest).set({ status: input.status, updatedAt: /* @__PURE__ */ new Date() }).where(eq(Guest.uniqueCode, input.uniqueCode));
  ...
  seating: {
    assign: defineAction({
      ...
      handler: async (input) => {
        ...
        await db.update(Guest).set({ tableId: input.tableId, updatedAt: /* @__PURE__ */ new Date() }).where(eq(Guest.id, input.guestId));
        return { success: true };
      }
    })
  },
  ...
  settings: {
    update: defineAction({
      ...
      input: z.object({
        ...
        ceremonyAddress: z.string(),
        receptionVenue: z.string(),
      ...
      handler: async (input) => {
        await db.update(Settings).set({ ...input, updatedAt: /* @__PURE__ */ new Date() }).where(eq(Settings.id, 1));
        return { success: true };
      }
    })
  },
  ...
  guests: {
    ...
    resetStatus: defineAction({
      ...
      handler: async (input) => {
        await db.update(Guest).set({ status: "pending", updatedAt: /* @__PURE__ */ new Date() }).where(eq(Guest.id, input.guestId));
        return { success: true };
      }
    })
  },
